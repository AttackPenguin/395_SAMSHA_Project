...
import numpy as np
import pandas as pd                                    #
from collections import Counter                                                ## Yoshi 
from sklearn.experimental import enable_iterative_imputer                      ## Yoshi
from sklearn.impute import IterativeImputer                                    ## Yoshi
from sklearn import preprocessing

data_directory = "//Users/ybird/Documents/Machine_Learning/DATASETS"


def get_data(data_directory: str = data_directory):
    # Import raw data
    raw_data = get_2019_samhsa_data(data_directory)
    ...
    med_assisted_opiod_tx = get_med_assisted_opiod_tx_encoding(raw_data)

    # Add Missing Values feature
    missing_values = get_missings(raw_data)                  # Added function call to missing_values YOSHI
    ...
    features = np.concatenate((
        ...
        tx_setting, med_assisted_opiod_tx       # Added missing_values column YOSHI
    ), axis=1)

    return features, targets, missing_values


def build_clean_df(data, features, threshold):                                    ## START YOSHI ##
    """ 
    Pass in the raw uploaded pandas dataframe, a list of features, and a           ##   |
    tow-wise missing count threshold, and it will return a dataframe containing    ##   |
    only the columns from the original dataframd labeled with your desired         ##   |
    features. Adds a column of missing counts for each row from among the          ##   |
    features in your feature list, then removes any rows from the data where       ##   V
    the missing count is higher than the threshold value.
       
    :param data: pd.DataFrame, list, int
    :return: pd.DataFrame       
    """
    
    data_copy = pd.DataFrame(columns=features)
    for i in features:
        data_copy[i] = data[i]
    data_copy['MISSCOUNT'] = get_missings(data_copy)
    data_copy = data_copy.loc[data_copy['MISSCOUNT'] < threshold]
    return data_copy
    ## END YOSHI ##


def get_med_assisted_opiod_tx_encoding(data: pd.DataFrame):
    """
...
def get_2019_samhsa_data(dir: str) -> pd.DataFrame:
    """
...
def get_missings(data: pd.DataFrame):                                              ## START YOSHI ##
    """                                                                            ##  |
    Pass in the raw uploaded pandas dataframe, and it will return a column         ##  |
    ...
    for index, row in enumerate(data.itertuples()):
        counter = Counter(row)
        if -9 not in counter.keys():
            missing_features[index] = 0
        else:
            missing_features[index] = counter[-9]
        list_of_counters.append(counter)
    return missing_features
    ##   END YOSHI  ##


def impute_missings(data: pd.DataFrame):
    """
    Pass in raw uploaded pandas dataframe, and it will return a copy of the        ## START YOSHI ##
    dataframe with values imputed using multivariate feature imputation without    ##  |
    modifying the original dataframe. NOTE: Must use one-hot conversion prior      ##  |
    to use! Otherwise, imputed data is meaningless.                                ##  |

    :param data: pd.DataFrame                                                      ##  |
    :return: pd.DataFrame                                                          ##  |

    """                                                                            ##  V
    impute_copy = data.copy()
    impute_copy_clean = impute_copy.replace({-9: np.nan})    
    imp = IterativeImputer(max_iter=10, random_state=0, missing_values = np.nan, imputation_order = 'ascending')
    imp.fit(impute_copy)
    data_with_imputations = imp.transform(impute_copy_clean)
    print(np.round(data_with_imputations))
    print(type(data_with_imputations))                                             ## END YOSHI    ##
    return data_with_imputations
    

df = get_2019_samhsa_data(data_directory)
test_impute = build_clean_df(df.loc[0:1000], ['AGE', 'GENDER', 'RACE', 'ETHNIC', 'MARSTAT', 'EDUC', 'EMPLOY', 'VET', 'SERVICES'], 4)
print(test_impute.shape)
data_with_imputations = impute_missings(test_impute)
