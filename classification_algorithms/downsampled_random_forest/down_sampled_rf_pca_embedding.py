...
def main():
    ...
    # downsampled_rf_pca_embedding(
    #     feature_model_A_1,
    #     'down_sampled_rf_pca_embedding_fm_A_1',
    #     'Down Sampled RF with PCA Embedding, Feature Model A_1'
    # )
    ...
    downsampled_rf_pca_embedding(
        feature_model_D_1,
        'down_sampled_rf_pca_embedding_fm_D_1',
        'Down Sampled RF with PCA Embedding, Feature Model D_1'
    )
    run_time = (pd.Timestamp.now() - start_time).total_seconds() / 60.0
    print(f"Training Completed in {run_time:.2f} minutes.\n")
    ...
    downsampled_rf_pca_embedding(
        feature_model_D_2,
        'down_sampled_rf_pca_embedding_fm_D_2',
        'Down Sampled RF with PCA Embedding, Feature Model D_2'
    )
    run_time = (pd.Timestamp.now() - start_time).total_seconds() / 60.0
    print(f"Training Completed in {run_time:.2f} minutes.\n")
    ...
    downsampled_rf_pca_embedding(
        feature_model_D_3,
        'down_sampled_rf_pca_embedding_fm_D_3',
        'Down Sampled RF with PCA Embedding, Feature Model D_3'
    )
    run_time = (pd.Timestamp.now() - start_time).total_seconds() / 60.0
    print(f"Training Completed in {run_time:.2f} minutes.\n")
    ...
    downsampled_rf_pca_embedding(
        feature_model_D_4,
        'down_sampled_rf_pca_embedding_fm_D_4',
        'Down Sampled RF with PCA Embedding, Feature Model D_4'
    )
    run_time = (pd.Timestamp.now() - start_time).total_seconds() / 60.0
    print(f"Training Completed in {run_time:.2f} minutes.\n")
...
def downsampled_rf_pca_embedding(feature_model,
                                 filename: str,
                                 filename_pretty: str):
    """
    ...
    Performs PCA processing of features prior to analysis, not to decrease the
    number of features to improve performance, but to attempt to embed
